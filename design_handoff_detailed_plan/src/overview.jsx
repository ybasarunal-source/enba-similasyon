// Genel Bakış (Overview) — KPIs, charts, scenario summary, budget vs actual snapshot
const { useMemo: _useM_overview } = React;
const RC = window.Recharts;

const OverviewPanel = ({ scenarioId, periodGranularity }) => {
  const scen = SCENARIOS[scenarioId];
  const cc = useChartColors();
  const series = _useM_overview(() => buildSeries(scen), [scenarioId]);

  // aggregate to the user's chosen granularity for the chart only
  const grouped = _useM_overview(() => {
    ...
    for (let i = 0; i < series.length; i += size) {
      ...
      const label = periodGranularity === 'quarter'
        ...
        : `${chunk[0].y}`;
      out.push({
        label, key: chunk[0].key, idx: chunk[0].idx,
  ...
  // Totals across 24m horizon
  const totals = _useM_overview(() => {
    const t = series.reduce((a, x) => ({
      revenue: a.revenue + x.revenue,
      opex: a.opex + x.opex,
      ebitda: a.ebitda + x.ebitda,
      net: a.net + x.net,
  ...
  const cashCum = _useM_overview(() => {
    const investment = 4_800_000; // örnek başlangıç yatırımı
    let cum = -investment;
    let paybackIdx = null;
  ...
  // Year-over-year revenue trend for sparkline
  const revSpark = grouped.map(g => g.revenue);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-4">
        <KpiCard
          label="Toplam Gelir (24 ay)"
          value={fmtTL(totals.revenue)}
          sub="Ürün + hizmet satırları toplamı"
          trend={0.182}
          accent="orange"
          icon={<I.Revenue size={14}/>}
          tooltip="Seçili senaryo altında planlanan toplam ciro"
        />
        <KpiCard
          label="EBITDA"
          value={fmtTL(totals.ebitda)}
          sub={`Marj ${fmtPct(ebitdaMargin)} · Faaliyet Karı`}
          trend={0.241}
          accent="green"
        ...
        <KpiCard
          label="Net Kâr"
          value={fmtTL(totals.net)}
          sub="Amortisman + vergi sonrası"
      ...
      <Card>
        <SectionTitle
          eyebrow={`${scen.label} senaryo`}
          title="Gelir & Gider Projeksiyonu"
          action={
        ...
        <div className="h-[280px]">
          <RC.ResponsiveContainer width="100%" height="100%">
            <RC.ComposedChart data={grouped} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
              <defs>
              ...
              <RC.CartesianGrid strokeDasharray="2 4" stroke={cc.grid} vertical={false}/>
              <RC.XAxis dataKey="label" tickLine={false} axisLine={false}/>
              <RC.YAxis tickFormatter={(v) => v >= 1_000_000 ? (v/1_000_000).toFixed(1)+'M' : (v/1000).toFixed(0)+'K'}
                tickLine={false} axisLine={false} width={48}/>
              <RC.Tooltip
                cursor={{ fill: 'rgba(227,82,5,0.05)' }}
                formatter={(v, name) => [fmtTL(v), name]}
              />
              <RC.Area type="monotone" dataKey="revenue" stroke="#E35205" strokeWidth={2.5} fill="url(#revGrad)" name="Gelir"/>
              <RC.Line type="monotone" dataKey="opex" stroke={cc.muted} strokeWidth={1.8} strokeDasharray="4 3" dot={false} name="Toplam Gider"/>
              <RC.Line type="monotone" dataKey="ebitda" stroke="#3DBE7C" strokeWidth={2} dot={false} name="EBITDA"/>
            </RC.ComposedChart>
      ...
      <div className="grid grid-cols-12 gap-4">
        <Card className="col-span-7">
          <SectionTitle
            eyebrow="Kümülatif"
            title="Nakit Akışı (Birikimli)"
            action={
              <div className="text-[11.5px] text-enba-muted">
                Başlangıç yatırımı <span className="text-enba-text tabular">{fmtTL(cashCum.investment)}</span>
              </div>
            }
          />
          <div className="h-[220px]">
            <RC.ResponsiveContainer width="100%" height="100%">
              <RC.AreaChart data={cashCum.points} margin={{ top: 10, right: 12, left: 0, bottom: 0 }}>
                <defs>
                ...
                <RC.CartesianGrid strokeDasharray="2 4" stroke={cc.grid} vertical={false}/>
                <RC.XAxis dataKey="label" tickLine={false} axisLine={false} interval={2}/>
                <RC.YAxis tickFormatter={(v) => (v/1_000_000).toFixed(1)+'M'} tickLine={false} axisLine={false} width={48}/>
                <RC.ReferenceLine y={0} stroke={cc.refLine} strokeDasharray="3 3"/>
                {cashCum.paybackIdx != null && (
                  <RC.ReferenceLine x={cashCum.points[cashCum.paybackIdx].label}
                    stroke="#3DBE7C" strokeDasharray="3 3"
                    label={{ value: 'Geri Ödeme', position: 'top', fill: '#3DBE7C', fontSize: 10 }}/>
                )}
                <RC.Tooltip formatter={(v) => [fmtTL(v), 'Birikimli Nakit']}/>
                <RC.Area type="monotone" dataKey="cash" stroke={scen.color} strokeWidth={2} fill="url(#cashGrad)"/>
              </RC.AreaChart>
            </RC.ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card className="col-span-5">
                ...
                <div key={s.id}
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="text-[13px] font-medium" style={{ color: active ? s.color : '#EDEDED' }}>{s.label}</div>
          ...
          {/* Quick budget-vs-actual snapshot */}
      ...
      <div className="grid grid-cols-12 gap-4">
          ...
          <div className="h-[230px] px-2">
            <RC.ResponsiveContainer width="100%" height="100%">
              <RC.BarChart data={
                ...
                <RC.CartesianGrid strokeDasharray="2 4" stroke={cc.grid} vertical={false}/>
                <RC.XAxis dataKey="label" tickLine={false} axisLine={false}/>
                <RC.YAxis tickFormatter={(v) => (v/1000).toFixed(0)+'K'} tickLine={false} axisLine={false} width={42}/>
                <RC.Tooltip formatter={(v, k) => [fmtTL(v), PRODUCTS.find(p => p.id === k)?.name || k]}/>
                {PRODUCTS.map((p) => (
                  <RC.Bar key={p.id} dataKey={p.id} stackId="rev" fill={p.color}/>
                ))}
              </RC.BarChart>
            </RC.ResponsiveContainer>
          </div>
        ...
        <Card className="col-span-4">
          ...
          <div className="space-y-3">
            <ActivityItem
              icon={<I.Edit size={12}/>} tone="orange"
              who="Murat K." what="PET Granül (Şeffaf) fiyatını ₺27.500 → ₺28.500 olarak güncelledi."
              when="2 saat önce"/>
            <ActivityItem
              icon={<I.Plus size={12}/>} tone="green"
              who="Ayşe T." what="Yeni gider kalemi: 'Sigorta & Yasal' eklendi."
              when="Dün"/>
            <ActivityItem
              icon={<I.Sparkles size={12}/>} tone="blue"
              who="Sistem" what="İyimser senaryo katsayıları yenilendi (+%12 gelir varsayımı)."
              when="2 gün önce"/>
            <ActivityItem
              icon={<I.Check size={12}/>} tone="green"
              who="Onay" what="2025 Q1 bütçesi finalize edildi ve kilitlendi."
              when="3 gün önce"/>
            <ActivityItem
              icon={<I.Refresh size={12}/>} tone="amber"
              who="Murat K." what="Atık Toplama Hizmeti hacim varsayımı %18 → %20 (yıllık)."
              when="5 gün önce"/>
          </div>
        </Card>
      </div>
        ...
        return (
          <div key={r.label} className="flex items-center gap-3">
            ...
            <div className="w-[80px] text-right">
              <Variance value={r.isExpense ? -diff : diff} asPct={false}/>
            </div>
          </div>
        );
      })}
...
const ActivityItem = ({ icon, who, what, when, tone = 'orange' }) => {
  const tones = {
    orange: 'bg-enba-orange/15 text-enba-orange',
    green:  'bg-enba-green/15 text-enba-green',
    blue:   'bg-enba-blue/15 text-enba-blue',
    amber:  'bg-enba-amber/15 text-enba-amber',
  };
  return (

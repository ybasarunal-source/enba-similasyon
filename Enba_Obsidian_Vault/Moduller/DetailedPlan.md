...
1. ~~**Panel hesapları gerçek mi?**~~ → **Çözüldü.** Wizard verisi PlanCtx'e doğru akıyor; tüm paneller `usePlanData()` ile gerçek veriyi okuyor.

2. ~~**startOffset / endOffset etkisi**~~ → **Çözüldü.** `Product` arayüzüne `startOffset`/`endOffset` eklendi. Shell ctxValue proje offset'lerini tüm gelir ve gider kalemlerine yansıtıyor. `revenueFor()` startOffset/endOffset kontrolü yapıyor.

3. ~~**CashFlow periods uyumsuzluğu**~~ → **Çözüldü.** CashFlowPanel artık kendi `buildMonths()` monthly period serisini inşa ediyor; PlanCtx granularity'sine bağımlılık kaldırıldı.

4. ~~**OverviewPanel hardcoded investment ₺4,8 Mn**~~ → **Çözüldü.** cashEvents.type='investing' toplamından hesaplanıyor; girilmemişse "Yatırım nakit olayı girilmemiş" gösteriyor.

5. ~~**BvaSnapshot mock veri**~~ → **Çözüldü.** actualsThrough === 0 ise boş durum; > 0 ise `bvaForPeriod()` ile gerçek hesap.

### 🔴 Kritik (sonraki adım)

1. **CostCenter maliyeti projeye akıyor mu?**
   `facilityShareFor(project, allProjects, cc)` hesabı — aynı gider merkezine bağlı birden fazla aktif proje varsa `allocationWeight` oranında bölüşüm yapılıyor. Shell'de bu paylaşımın doğru yansıdığı test edilmeli. (Hesap kodu doğru görünüyor, gerçek veriyle doğrulama bekliyor.)
...
## Granül Üretimi — Gerçek Veri (2026-05-23)

Kaynak: [[Ham-Kaynaklar/2026-05-GranulUretimi-IsPlan|İş Planı Konuşması]]  
Parametreler: [[Kararlar/2026-05-GranulUretimi-Parametre|Granül Üretimi Parametreleri]]

### Modüle doğrudan giren veriler

**CostCenter (Granül Tesisi):** 169.500 TL/ay sabit  
Kira 65K + Forklift 30K + Bakım 30K + Muhasebe 20K + Çevre müh. 18K + İnternet 5K + Elektrik müh. 1,5K

**Proje giderleri (110 ton senaryosu):**
- Hammadde: 2.090.000 TL/ay (M369)
- İşçilik: 332.000 TL/ay (M489 — 5 işçi + 1 usta)
- Enerji: ~300-400K TL/ay (M405, makine-makine formül)
- Atık su: 88.000 TL/ay (M529)
- Çuval: ~22.000 TL/ay (M999)

**Gelirler:**
- 1. kalite granül: 93,5 ton × 40.000 = 3.740.000 TL/ay
- 2. kalite ürün: 8,25 ton × 30.000 = 247.500 TL/ay

### Modül boşlukları (bu plandan tespit edildi)

| Boşluk | Aciliyet |
|--------|----------|
| Hacim-tabanlı senaryo (ton/ay input değişkeni) | 🔴 Kritik — mevcut ±% çarpan yetmiyor |
| Birim değişken maliyet (TL/ton × hacim) | 🟡 Önemli — şu an aylık sabit rakam girilmeli |
| Başabaş tonu (ton cinsinden, TL/kg değil) | 🟡 Önemli — OverviewPanel'e eklenebilir |
| Makine parametresi (kW, kapasite, DF) | 🟢 İleride — enerji el hesabıyla girilebiliyor |

...
function DetayliPlanWizard({ initialData, onSave, onCancel }) {
    ...
    const detayliPdfExport = () => {
        ...
        const fmtN = v => Number(v||0).toLocaleString('tr-TR',{maximumFractionDigits:0});
...
function DetayliPlanModulu({ navigate, bekleyenPlanlar, setBekleyenPlanlar, aktifPlanlar, setAktifPlanlar }) {
    ...
    const birlestirmePdfExport = () => {
        ...
        const tdKf = (v, pos) => `<td style="padding:4px 4px;font-size:8px;text-align:right;color:${v==null?'#bbb':pos?'#1a7f4b':'#c0392b'};background:#f0f4ff;font-weight:${v==null?400:600};border:1px solid #e0e0e0;white-space:nowrap">${v!=null?Number(v).toLocaleString('tr-TR'):'—'}</td>`;
                                                        ...
                                                        {[
                                                            ...
                                                            <tr key={row.label} style={row.bold ? {background:'rgba(79,195,247,0.04)', borderBottom:'1px solid rgba(79,195,247,0.15)'} : {}}>
                                                                ...
                                                                {Array.from({length:12}, (_, i) =>
                                                                    aktifPlanlar.map((p, pi) => {
                                                                        const r = row.get(p, i);
                                                                        return <td key={`${i}-${pi}`} style={{color: r ? (r.color || '#A0AEC0') : '#333', borderLeft: pi===0 ? '1px solid rgba(255,255,255,0.08)' : 'none', fontSize:'11px', fontWeight: r?.bold ? 700 : 'normal'}}>{r && r.val != null ? r.val.toLocaleString('tr-TR') : '-'}</td>;
                                                                    })
                                                                )}
                                                                {aktifPlanlar.map((p, pi) => {
                                                                    const t = row.total(p);
                                                                    return <td key={pi} className="total-col" style={{color: t.color || '#A0AEC0', borderLeft: pi===0 ? '2px solid rgba(255,255,255,0.15)' : 'none', fontSize:'11px', fontWeight: t.bold ? 700 : 'normal'}}>{t.val != null ? t.val.toLocaleString('tr-TR') : '-'}</td>;
                                                                })}
                                                            </tr>
                                                        ))}
                                                        <tr style={{height:'8px'}}><td colSpan={totalCols} style={{border:'none'}}></td></tr>

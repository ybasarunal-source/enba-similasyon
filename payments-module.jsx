...
function PaymentsModule() {
    ...
    const cardStyle = { background: '#fff', borderRadius: '1.2rem', padding: '24px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', border: '1px solid #eee' };
    ...
    const badgeStyle = (type, status) => ({
        padding: '3px 10px', borderRadius: '2rem', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase',
        background: status === 'paid' ? '#E8F5E9' : status === 'pending' ? '#FFF3E0' : '#FFEBEE',
        color: status === 'paid' ? '#2E7D32' : status === 'pending' ? '#E65100' : '#C62828'
    });

    const ProjectionChart = () => {
        if (projectionData.length === 0) return <div>{window.t('payments.no_projection_data')}</div>;
        ...
        return (
            <div style={{ width: '100%', height: '220px', position: 'relative', marginTop: '20px' }}>
                <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: '100%', overflow:'visible' }}>
                    {/* Zero line */}
                    <line x1="0" y1={getY(0)} x2={W} y2={getY(0)} stroke="#ddd" strokeDasharray="5,5" />
                    {/* Balance Path */}
                    <path d={path} fill="none" stroke="var(--enba-dark)" strokeWidth="3" />
                    {/* Points */}
                    {projectionData.map((d, i) => (
                        <circle key={i} cx={(i+1)*step} cy={getY(d.balance)} r="4" fill={d.isNegative ? '#e74c3c' : 'var(--enba-dark)'} />
                    ))}
                </svg>
    ...
    return (
        <div style={containerStyle}>
            <div style={headerStyle}>
                <div>
                    <h2 style={{ margin: 0, color: 'var(--enba-dark)', fontSize: '24px' }}>⚡  {window.t('payments.upcoming')}</h2>
                    <p style={{ color: '#666', fontSize: '14px', marginTop: '4px' }}>{window.t('payments.desc')}</p>
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                    <div style={{ background: '#eee', padding: '4px', borderRadius: '0.8rem', display: 'flex' }}>
                        <button onClick={() => setViewMode('list')} style={{ padding: '8px 16px', border: 'none', borderRadius: '0.6rem', background: viewMode === 'list' ? '#fff' : 'transparent', fontWeight: 600, cursor: 'pointer' }} title={window.t('payments.list')}>⚡ </button>
                        <button onClick={() => setViewMode('calendar')} style={{ padding: '8px 16px', border: 'none', borderRadius: '0.6rem', background: viewMode === 'calendar' ? '#fff' : 'transparent', fontWeight: 600, cursor: 'pointer' }} title={window.t('payments.calendar')}>⚡ </button>
                        <button onClick={() => setViewMode('projection')} style={{ padding: '8px 16px', border: 'none', borderRadius: '0.6rem', background: viewMode === 'projection' ? '#fff' : 'transparent', fontWeight: 600, cursor: 'pointer' }} title={window.t('payments.cash_projection')}>⚡ </button>
                    </div>
            ...
            <div style={{ ...cardStyle, marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '20px', background: 'var(--enba-dark)', color: '#fff' }}>
                <div style={{ fontSize: '24px' }}>⚡ </div>
                <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '12px', opacity: 0.8, fontWeight: 600 }}>{window.t('payments.initial_cash').toUpperCase()}</div>
                        ...
                        <tbody>
                            {payments.sort((a,b) => new Date(a.dueDate) - new Date(b.dueDate)).map(p => (
                                <tr key={p.id} style={{ borderBottom: '1px solid #f9f9f9' }}>
                                    ...
                                    <td style={{ padding: '12px', fontSize: '13px', color: '#666' }}>{window.t(`payments.${p.category.toLowerCase()}`) || p.category}</td>
                                    <td style={{ padding: '12px', fontWeight: 700, color: p.type === 'incoming' ? '#27ae60' : '#e74c3c' }}>
                                    ...
                                    <td style={{ padding: '12px' }}>
                                        <button onClick={() => { setEditingItem(p); setIsModalOpen(true); }} style={{ background: 'none', border: 'none', cursor: 'pointer', marginRight: '10px' }}>✏️</button>
                                        <button onClick={() => deletePayment(p.id)} style={{ background: 'none', border: 'none', cursor: 'pointer' }}>⚡ ️</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            {viewMode === 'calendar' && (
                <div style={cardStyle}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <button onClick={() => changeMonth(-1)} style={{ ...btnPrimary, background: '#eee', color: '#333' }}>◀ {window.t('common.previous')}</button>
                        <h3 style={{ margin: 0, textTransform: 'capitalize' }}>{currentDate.toLocaleString(localStorage.getItem('enba_lang') === 'TR' ? 'tr-TR' : 'en-US', { month: 'long', year: 'numeric' })}</h3>
                        <button onClick={() => changeMonth(1)} style={{ ...btnPrimary, background: '#eee', color: '#333' }}>{window.t('common.next')} ▶</button>
                    </div>
                                    ...
                                    <>
                                        ...
                                        {cell.payments.map(p => (
                                            <div key={p.id} style={{ 
                                                ...
                                                background: p.type === 'incoming' ? 'rgba(39, 174, 96, 0.1)' : 'rgba(231, 76, 60, 0.1)',
                                                color: p.type === 'incoming' ? '#27ae60' : '#e74c3c',
                                                whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                                                borderLeft: `3px solid ${p.type === 'incoming' ? '#27ae60' : '#e74c3c'}`
                                            }} title={p.title}>
            ...
            {viewMode === 'projection' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    <div style={cardStyle}>
                        <h3 style={{ margin: 0 }}>⚡  {window.t('payments.cash_projection')}</h3>
                        <ProjectionChart />
                    </div>

                    {negativeDates.length > 0 && (
                        <div style={{ ...cardStyle, borderLeft: '10px solid #e74c3c', background: '#fdf2f2' }}>
                            <h4 style={{ color: '#c0392b', margin: '0 0 10px 0' }}>⚠️ {window.t('payments.negative_alert')}</h4>
                            <p style={{ fontSize: '14px', color: '#666' }}>{window.t('payments.negative_alert_desc')}</p>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '10px', marginTop: '15px' }}>
                                {negativeDates.map((d, i) => (
                                    <div key={i} style={{ padding: '10px', borderRadius: '8px', background: '#fff', border: '1px solid #fab1a0', textAlign: 'center' }}>
                                        <div style={{ fontWeight: 800, color: '#e74c3c' }}>{d.date}</div>
                    ...
                    {negativeDates.length === 0 && (
                        <div style={{ ...cardStyle, borderLeft: '10px solid #27ae60', background: '#f2f9f5' }}>
                            <h4 style={{ color: '#27ae60', margin: 0 }}>{window.t('payments.cash_safe')}</h4>
                            <p style={{ fontSize: '14px', color: '#666', marginTop: '5px' }}>{window.t('payments.cash_safe_desc')}</p>
                        </div>
                    )}
                </div>
            )}
                    ...
                    <div style={{ ...cardStyle, width: '450px' }}>
                        ...
                        <form onSubmit={(e) => {
                            ...
                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                                <button type="button" onClick={() => setIsModalOpen(false)} style={{ ...btnPrimary, background: '#eee', color: '#333' }}>{window.t('common.cancel')}</button>
